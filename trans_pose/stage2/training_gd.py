...
def compute_pose_loss_multi(offsets, points, point_labels, kpts_dict_batch):
    """
    NEW: Pose loss using mean-shift clustering on voted keypoints.
    
    For each object:
    1. Extract votes (points + offsets) for that object (using 2D mask labels)
    2. Apply mean-shift clustering → predicted keypoints
    3. Compare predicted keypoints to GT keypoints using MSE
    
    Args:
        offsets: [B, N, K, 3] predicted offsets
        points: [B, N, 3] 3D point cloud
        point_labels: [B, N] from 2D mask (used for object grouping)
        kpts_dict_batch: List[Dict[str, np.ndarray]] - GT keypoints (transformed)
    ...
    # Compute votes: points + offsets → [B, N, K, 3]
    ...
    for b in range(B):
        # Get unique object IDs (from 2D mask labels)
        unique_objs = torch.unique(point_labels[b])
        ...
        for obj_id in unique_objs:
            ...
            # Get votes for this object (using 2D mask labels for grouping)
            mask_obj = (point_labels[b] == obj_id_int)  # [N]
            
            if mask_obj.sum() == 0:
                continue
            
            votes_obj = votes[b, mask_obj]  # [M, K, 3]
            
            # Mean-shift clustering to aggregate votes
            votes_input = votes_obj.unsqueeze(0)  # [1, M, K, 3]
            pred_kpts = mean_shift_clustering(votes_input, bandwidth=0.05, num_iters=15)  # [1, K, 3]
            pred_kpts = pred_kpts.squeeze(0)  # [K, 3]
            
            # Load GT keypoints (transformed)
            target_kpts = torch.tensor(kpts_dict[obj_id_str], device=device, dtype=torch.float32)
            
            if target_kpts.shape[0] != K:
                continue
            
            # MSE loss between predicted and GT keypoints
            loss_obj = F.mse_loss(pred_kpts, target_kpts)
            total_loss += loss_obj
            num_objects_processed += 1
...
def run_epoch(model, dataloader, intrinsics_tuple_scaled, device, optimizer=None, is_train=True, epoch_idx=0):
    ...
    for step, data in enumerate(pbar):
        ...
        with torch.set_grad_enabled(is_train):
            ...
            # Total loss (NEW WEIGHTS)
            loss_total = (W_POSE * loss_pose + 
                         W_SEG * loss_seg + 
                         W_OFFSET * loss_off + 
                         W_REG * loss_reg)
    ...
    return (total_loss / num_batches, 
            total_pose / num_batches,  # ← NEW
            total_seg / num_batches, 
            total_off / num_batches,
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        train_loss, train_pose, train_seg, train_off, train_reg = run_epoch(
            model, train_loader, intrinsics_tuple_scaled, device,
            optimizer, is_train=True, epoch_idx=epoch
        )
        val_loss, val_pose, val_seg, val_off, val_reg = run_epoch(
        ...
        print(f"   Train: Loss={train_loss:.4f}, Pose={train_pose:.4f}, Seg={train_seg:.4f}, Off={train_off:.4f}")
        print(f"   Valid: Loss={val_loss:.4f}, Pose={val_pose:.4f}, Seg={val_seg:.4f}, Off={val_off:.4f}")
